...
#   ./package.sh --dev     dev     → Perch-dev.app / com.perch.perch.dev
#
...
# would show two indistinguishable "perch" entries. The dev variant
# gets its own bundle id + display name "perch (dev)" so each side
...
cp .build/release/perch "$APP/Contents/MacOS/perch"   # = CFBundleExecutable
